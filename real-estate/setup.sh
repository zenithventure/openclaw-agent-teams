...
if [[ "${1:-}" == "--help" ]]; then
    echo "Setup script for Real Estate team (bare-metal / self-hosted OpenClaw)"
    echo ""
    echo "Usage:"
...
AGENT_NAMES=("DealMaker" "Analyst" "Coordinator" "Underwriter")
AGENT_ROLES=("Deal Sourcing & Negotiation" "Market Analysis & Valuation" "Transaction Coordination" "Risk Assessment & Due Diligence")
...
banner() {
    ...
    echo -e "${BOLD}║                                                       ║${NC}"
    echo -e "${BOLD}║  ${DIM}Deals · Analysis · Coordination · Underwriting${NC}${BOLD}      ║${NC}"
    echo -e "${BOLD}╚═══════════════════════════════════════════════════════╝${NC}"
    echo ""
}
...
const removeIds = ['red-dealmaker', 'yellow-analyst', 'green-coordinator', 'blue-underwriter'];
...
// Remove any existing Real Estate agents (idempotent)
const reIds = ['red-dealmaker', 'yellow-analyst', 'green-coordinator', 'blue-underwriter'];
existing.agents.list = existing.agents.list.filter(a => !reIds.includes(a.id));
...
const oldMission = /> \*\*Evaluate \[PROPERTY\/DEAL\][\s\S]*?supporting evidence\.\*\*/;
...
print_summary() {
    ...
    echo -e "  ${RED}● DealMaker${NC}    (Red)    — Sources deals, manages negotiations"
    echo -e "  ${YELLOW}● Analyst${NC}      (Yellow) — Market analysis, property valuation"
    echo -e "  ${GREEN}● Coordinator${NC}  (Green)  — Transaction coordination, communications"
    echo -e "  ${BLUE}● Underwriter${NC}  (Blue)   — Risk assessment, due diligence review"
    echo ""
    echo -e "${BOLD}Skills installed (8):${NC}"
    echo -e "  comp-analysis, deal-pipeline, document-review,"
    echo -e "  due-diligence, risk-assessment,"
    ...
    echo "  ./setup.sh --vision \"Evaluate 123 Main St — 24-unit multifamily ...\""
    echo ""
