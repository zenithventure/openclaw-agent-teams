...
if [[ "${1:-}" == "--help" ]]; then
    echo "Setup script for Recruiter team (bare-metal / self-hosted OpenClaw)"
    echo ""
    echo "Usage:"
...
AGENT_ROLES=("Talent Sourcing & Pipeline" "Interview Design & Evaluation" "Scheduling & Communication" "EEO Compliance & Policy")
...
banner() {
    ...
    echo -e "${BOLD}║                                                       ║${NC}"
    echo -e "${BOLD}║  ${DIM}Sourcing · Screening · Interviewing · Compliance${NC}${BOLD}    ║${NC}"
    echo -e "${BOLD}╚═══════════════════════════════════════════════════════╝${NC}"
    echo ""
}
...
const removeIds = ['red-headhunter', 'yellow-interviewer', 'green-coordinator', 'blue-compliance'];
...
// Remove any existing Recruiter agents (idempotent)
const recruiterIds = ['red-headhunter', 'yellow-interviewer', 'green-coordinator', 'blue-compliance'];
existing.agents.list = existing.agents.list.filter(a => !recruiterIds.includes(a.id));

// Add Recruiter agents
...
const oldMission = /> \*\*Fill \[ROLE TITLE\] at \[COMPANY\][\s\S]*?within \[TIMELINE\]\.\*\*/;
...
print_summary() {
    ...
    echo -e "  ${RED}● Headhunter${NC}   (Red)    — Sources candidates, manages talent pipeline"
    echo -e "  ${YELLOW}● Interviewer${NC}  (Yellow) — Designs interviews, evaluates candidates"
    echo -e "  ${GREEN}● Coordinator${NC}  (Green)  — Schedules interviews, manages communications"
    echo -e "  ${BLUE}● Compliance${NC}   (Blue)   — EEO compliance, policy review (read-only)"
    echo ""
    echo -e "${BOLD}Skills installed (8):${NC}"
    echo -e "  candidate-screening, interview-design, job-description,"
    echo -e "  pipeline-management, eeo-compliance,"
    ...
    echo "  ./setup.sh --vision \"Hire a Senior Backend Engineer at Acme Corp...\""
    echo ""
